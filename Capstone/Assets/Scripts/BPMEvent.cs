...
{
    [SerializeField]
    private float bpm; // 현재 심박수

    [SerializeField] private float average_bpm; // 평균 심박수
    
    private const float FIRST_TRUMPET = 1.05f; // 1단계 심박수 이벤트 상수
    private const float SECOND_TRUMPET = 1.12f; // 2단계 심박수 이벤트 상수
    ...
    void Update()
    {
        bpm = float.Parse(FTU.Instance.GetBpmValues(0)); // 현재 심박수 값 가져오기
        BPMLevel();   
    }
    ...
    IEnumerator RestOn()
    {
        ...
        FTU.Instance.BPMEvent = 0; // 심박수 이벤트 초기화
